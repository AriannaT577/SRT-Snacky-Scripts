...
public class BackshotsTillBlueBalls extends LinearOpMode {
    ...
    public class Shooter {
        ...
        public class Shoot1 implements Action{
            ...
            public boolean run(@NonNull TelemetryPacket packet){
                ...
                shooterLeft.setVelocity(1400); //1800
                shooterRight.setVelocity(1400); //1800
                return false;
            }
        }
    ...
    public class ShooterAngle {
        ...
        public class angle1 implements Action{
            @Override
            public boolean run(@NonNull TelemetryPacket packet){
                shootAngle.setPosition(0.1788); //0.184
                return false;
            }
        }
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Pose2d postshootPose = new Pose2d (new Vector2d(0, -55),1.92);//after shooting
        Pose2d firsteatPose = new Pose2d (new Vector2d(-47, -34),  0);//after eating first
        Pose2d secondeatPose = new Pose2d (new Vector2d(-47, -9.5), 0);//after eat second
        Pose2d hpeatPose = new Pose2d (new Vector2d(-48, -60), 0);//after eatig hp
        ...
        TrajectoryActionBuilder firsteatballs = drive.actionBuilder(postshootPose) //34
                .setTangent(Math.PI)
                .splineToLinearHeading(new Pose2d(-17, -34, 0), Math.PI
                        , new TranslationalVelConstraint(200),null )
                .splineToLinearHeading(new Pose2d(-48, -34,  0), Math.PI
                        , new TranslationalVelConstraint(100),null )
                ;

        TrajectoryActionBuilder firstpewballs = drive.actionBuilder(firsteatPose)
                .setTangent((11*Math.PI)/6)
                .splineToSplineHeading(new Pose2d(0, -55, 1.92), 0
                        , new TranslationalVelConstraint(200),null )
                ;

        TrajectoryActionBuilder secondeatballs = drive.actionBuilder(postshootPose) //9
                .setTangent(Math.PI)
                .splineToLinearHeading(new Pose2d(-15, -9.5, 0), Math.PI
                        , new TranslationalVelConstraint(200 ),null )
                .splineToLinearHeading(new Pose2d(-48, -9.5, 0), Math.PI
                        , new TranslationalVelConstraint(100),null ) //60
                ;

        TrajectoryActionBuilder secondpewballs = drive.actionBuilder(secondeatPose)
                .setTangent((11*Math.PI)/6)
                .splineToLinearHeading(new Pose2d(0, -55, 1.92), Math.PI*3/2//shoot
                        , new TranslationalVelConstraint(200),null )
                ;
        ...
        TrajectoryActionBuilder hpeatballs = drive.actionBuilder(postshootPose)
                ...
                .splineToLinearHeading(new Pose2d(-46, -43, Math.PI/6), Math.PI
                        , new TranslationalVelConstraint(200),null )
                .setTangent(3*Math.PI/2)
                .splineToLinearHeading(new Pose2d(-46, -60, Math.PI/6), 3*Math.PI/2
                        , new TranslationalVelConstraint(40),null )

                .splineToLinearHeading(new Pose2d(-42, -60, 0), Math.PI*7/4
                        , new TranslationalVelConstraint(200),null )
        ...
        TrajectoryActionBuilder hppewballs = drive.actionBuilder(hpeatPose)
                .setTangent((11*Math.PI)/6)
                .splineToLinearHeading(new Pose2d(-2, -55, 1.93), Math.PI*3/2
                        , new TranslationalVelConstraint(200),null )
                ;

        TrajectoryActionBuilder park = drive.actionBuilder(postshootPose)
                .splineToLinearHeading(new Pose2d(0, -25, Math.PI/2), Math.PI*3/2
                        , new TranslationalVelConstraint(200),null )
                ;
